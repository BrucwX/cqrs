...
func newSeededData(t *testing.T) *query.Data {
	return query.NewData(newSeededStore(t))
}
...
func newSeededStore(t *testing.T) *test_memory.Data {
	t.Helper()

	store, _, err := test_memory.NewData(nil)
	if err != nil {
		t.Fatalf("test_memory.NewData() error = %v", err)
	}
