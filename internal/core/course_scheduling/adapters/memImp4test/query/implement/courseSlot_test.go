...
func TestCourseSlotQueryPage(t *testing.T) {
	...
	got, err := q.PageCourseSlots(context.Background(), 1, 10)
	if err != nil {
...
func TestCourseSlotQueryListByCourseID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := q.ListCourseSlotsByCourseID(context.Background(), tt.courseID)
			if err != nil {
...
func TestCourseSlotQueryReturnsSeededFields(t *testing.T) {
	...
	got, err := q.ListCourseSlotsByCourseID(context.Background(), "C001")
	if err != nil {
		t.Fatalf("ListByCourseID() error = %v", err)
	}
	if len(got) != 2 {
