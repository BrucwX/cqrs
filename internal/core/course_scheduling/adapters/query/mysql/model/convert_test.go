...
func TestRoundTrip(t *testing.T) {
	...
	t.Run("courseSlotChange", func(t *testing.T) {
		...
		back, err := CourseSlotChangeToDO(po)
		if err != nil {
			t.Fatal(err)
		}
...
func TestDirtyDataRejected(t *testing.T) {
	cases := []struct {
		...
		{"student: 手机号不合法", func() error {
			_, err := StudentToDO(&Student{ID: 1, Name: "x", Phone: "12345", Status: 1})
			return err
		}},
		{"classroom: 楼栋为空", func() error {
			_, err := ClassroomToDO(&Classroom{ID: "cl", Capacity: 30, Status: 1})
			return err
		}},
		{"course: 容量越界（enrolled > max）", func() error {
			_, err := CourseToDO(&Course{
		...
		{"courseSlot: 时间段倒挂", func() error {
			_, err := CourseSlotToDO(&CourseSlot{
		...
		{"courseSlotChange: 目标计划跨天", func() error {
			_, err := CourseSlotChangeToDO(&CourseSlotChange{
