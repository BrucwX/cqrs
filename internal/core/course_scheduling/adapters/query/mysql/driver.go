// 本文件是读侧连库的底座：怎么开连接池、怎么补 DSN、怎么把语句打到日志里。
//
...
func Open(source string) (*sql.DB, func(), error) {
	...
	cleanup := func() {
		if err := db.Close(); err != nil {
			log.Error("closing course_scheduling mysql query adapter", "err", err)
			return
		}
		log.Info("closing course_scheduling mysql query adapter")
	}
