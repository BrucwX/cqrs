...
// qualificationQuery 是 repoquery.QualificationQuery 的 MySQL 实现。
type qualificationQuery struct {
	data *mysql.Data
}

// 编译期断言：实现必须满足接口。
var _ repoquery.QualificationQuery = (*qualificationQuery)(nil)

// NewQualificationQuery 创建 MySQL 版授课资质查询。
func NewQualificationQuery(d *mysql.Data) repoquery.QualificationQuery {
	return &qualificationQuery{data: d}
}

// Page 分页查询授课资质列表，按 ID 升序。
func (q *qualificationQuery) Page(ctx context.Context, page, pageSize int) ([]*qualification.Qualification, error) {
...
// 顺序按第一条资质记录。
func (q *qualificationQuery) CourseTypesByTeacherID(ctx context.Context, teacherID int64) ([]*courseType.CourseType, error) {
	query := `
SELECT ` + help.Qualify(help.CourseTypeColumns, "ct") + `
 ...
 ORDER BY f.first_id`
	...
	return help.QueryAll(ctx, q.data.Conn(ctx), query, []any{teacherID}, help.ScanCourseType, model.CourseTypeToDO)
}

// TeachersByCourseTypeID 根据课程类型 ID 获取有授课资质的讲师列表。
//
// 同样不按资质状态过滤；去重，顺序按第一条资质记录。
func (q *qualificationQuery) TeachersByCourseTypeID(ctx context.Context, courseTypeID string) ([]*teacher.Teacher, error) {
	query := `
SELECT ` + help.Qualify(help.TeacherColumns, "t") + `
 ...
 ORDER BY f.first_id`
	...
	return help.QueryAll(ctx, q.data.Conn(ctx), query, []any{courseTypeID}, help.ScanTeacher, model.TeacherToDO)
}
...
SELECT EXISTS (
	...
	return help.Exists(ctx, q.data.Conn(ctx), query, courseID, teacherID)
}
