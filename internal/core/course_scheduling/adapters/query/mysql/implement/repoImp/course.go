...
// courseQuery 是 repoquery.CourseQuery 的 MySQL 实现。
type courseQuery struct {
	data *mysql.Data
}

// 编译期断言：实现必须满足接口。
var _ repoquery.CourseQuery = (*courseQuery)(nil)

// NewCourseQuery 创建 MySQL 版课程查询。
func NewCourseQuery(d *mysql.Data) repoquery.CourseQuery {
	return &courseQuery{data: d}
}

// Page 分页查询课程列表，按 ID 升序。
func (q *courseQuery) Page(ctx context.Context, page, pageSize int) ([]*course.Course, error) {
...
func (q *courseQuery) AvailableForStudent(ctx context.Context, studentID int64) ([]*course.Course, error) {
...
func (q *courseQuery) AvailableForTeacher(ctx context.Context, teacherID int64) ([]*course.Course, error) {
...
func (q *courseQuery) AvailableForClassroom(ctx context.Context, classroomID string) ([]*course.Course, error) {
...
func (q *courseQuery) available(ctx context.Context, busy string, args ...any) ([]*course.Course, error) {
	query := `
SELECT ` + help.Qualify(help.CourseColumns, "c") + `
 ...
 ORDER BY c.id`
	...
	return help.QueryAll(ctx, q.data.Conn(ctx), query, args, help.ScanCourse, model.CourseToDO)
}
