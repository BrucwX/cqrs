...
func newIntegrationData(t *testing.T) *mysql.Data {
	t.Helper()
...
func TestReadSource(t *testing.T) {
	// 留空 -> 回落到 source
	data, cleanup, err := mysql.NewData(&conf.Data{Database: &conf.Data_Database{
		Driver: "mysql",
	...
	if _, _, err := mysql.NewData(&conf.Data{Database: &conf.Data_Database{
		Driver:     "mysql",
		Source:     integrationDSN(),
...
func TestIntegrationPage(t *testing.T) {
	...
	t.Run("classroom", func(t *testing.T) {
		got, err := NewClassroomQuery(data).Page(ctx, 1, 10)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("student", func(t *testing.T) {
		got, err := NewStudentQuery(data).Page(ctx, 1, 2)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("teacher", func(t *testing.T) {
		got, err := NewTeacherQuery(data).Page(ctx, 1, 10)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("course_slot", func(t *testing.T) {
		got, err := NewCourseSlotQuery(data).Page(ctx, 1, 10)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("course_slot_change", func(t *testing.T) {
		got, err := NewCourseSlotChangeQuery(data).Page(ctx, 1, 10)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("course_enrollment", func(t *testing.T) {
		got, err := NewCourseEnrollmentQuery(data).Page(ctx, 1, 2)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("absence_record", func(t *testing.T) {
		got, err := NewAbsenceRecordQuery(data).Page(ctx, 1, 10)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("student_makeup", func(t *testing.T) {
		got, err := NewStudentMakeupQuery(data).Page(ctx, 1, 10)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("qualification", func(t *testing.T) {
		got, err := NewQualificationQuery(data).Page(ctx, 1, 2)
		if err != nil {
			t.Fatal(err)
		}
...
func TestIntegrationEnrollment(t *testing.T) {
	data := newIntegrationData(t)
	ctx := context.Background()
	q := NewCourseEnrollmentQuery(data)

	// 报名：101→C001、102→C002、103→C001(在读)、103→C002(已结业，不过滤)
	got, err := q.CoursesByStudentID(ctx, 103)
	if err != nil {
		t.Fatal(err)
	}
	assertStrings(t, "103 上过的课", courseIDs(got), []string{"C001", "C002"})

	students, err := q.StudentsByCourseID(ctx, "C001")
	if err != nil {
		t.Fatal(err)
	}
	...
	empty, err := q.CoursesByStudentID(ctx, 999)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationMakeup(t *testing.T) {
	data := newIntegrationData(t)
	ctx := context.Background()
	q := NewStudentMakeupQuery(data)

	got, err := q.CoursesByStudentID(ctx, 101)
	if err != nil {
		t.Fatal(err)
	}
	...
	got, err = q.CoursesByStudentID(ctx, 102)
	if err != nil {
		t.Fatal(err)
	}
