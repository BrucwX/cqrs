...
//
//	mysql.go    本文件：读侧连接（Data / NewData / Conn）
//	driver.go   连库底座：Querier 抽象、DSN 归一化、Open、SQL 日志
//	model/      数据模型（PO）+ DO <-> PO 转换函数
//	implement/  查询操作的实现：domain/repo/query 的 10 个接口
//
// ⚠️ 读侧刻意不提供事务。如果 read_source 指向从库，那么命令侧在 WithTx
// 里通过查询仓库发起的读会落到从库的快照上，与写事务不构成原子读 ——
